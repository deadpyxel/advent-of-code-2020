...
def _calc_product(input_list: Iterable[int]) -> int:
    result = 1
    for x in input_list:
        result *= x
    return result
...
def solve_day01() -> None:
    ...
    for possibility in possible_values:
        print(f"Possible answer for part1: {_calc_product(possibility)}")
    ...
    for possibility in possible_values:
        print(f"Possible answer for part2: {_calc_product(possibility)}")
